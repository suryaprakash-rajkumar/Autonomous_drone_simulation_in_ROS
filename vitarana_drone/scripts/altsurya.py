...
class Edrone():
    ...
    def __init__(self):
        ...
        #
        self.n=0
        self.errors_PID=[0.0,0.0,0.0]
        self.prev_error=[0.0,0.0,0.0]
        self.prev_values=[0.0,0.0,0.0]
        self.max_values=[1023.0,1023.0,1023.0,1023.0]
        self.min_values=[0.0,0.0,0.0,0.0]
        #self.sum_errors=[0.0,0.0,0.0]
        self.iterm=[0.0,0.0,0.0]
        self.func_pid=[0.0,0.0,0.0]
        ...
        self.rollerrorpun = Float32()
        self.pitcherrorpub = Float32()
        self.zeroerrorpub = Float32()
    ...
    def roll_set_pid(self, roll):
        self.Kp[0] = roll.Kp * 0.018  # This is just for an example. You can change the ratio/fraction value accordingly
    ...
    def pitch_set_pid(self, pitch):
        self.Kp[1] = pitch.Kp * 0.018  # This is just for an example. You can change the ratio/fraction value accordingly
    ...
    def pid(self):
        ...
        # Converting quaternion to euler anglesself.func_pid[0]= (self.Kp[0] * self.errors_PID[0]) + ( self.Kd[0] *(self.errors_PID[0]-self.prev_error[0])) + (self.Ki[0]*self.iterm[0])
        ...
        self.drone_orientation_euler[0]=self.drone_orientation_euler[0]*100
        self.drone_orientation_euler[1]=self.drone_orientation_euler[1]*100
        self.drone_orientation_euler[2]=self.drone_orientation_euler[2]*100
        
        # Convertng the range from 1000 to 2000 in the range of -10 degree to 10 degree for roll axis
        self.setpoint_euler[0] = self.setpoint_cmd[0] * 0.02 - 30
        self.setpoint_euler[1] = self.setpoint_cmd[1] * 0.02 - 30
        self.setpoint_euler[2] = self.setpoint_cmd[2] * 0.02 - 30
        self.setpoint_base_speed= ((self.setpoint_cmd[3] -1000)/1000)*1024

        self.errors_PID[0]  = self.setpoint_euler[0]-self.drone_orientation_euler[0]
        self.errors_PID[1]  = self.setpoint_euler[1]-self.drone_orientation_euler[1]
        self.errors_PID[2]  = self.setpoint_euler[2]-self.drone_orientation_euler[2]
        print(self.errors_PID)
        #self.n = self.n +1
        self.iterm[0]=self.iterm[0]+self.errors_PID[0]
        self.iterm[1]=self.iterm[1]+self.errors_PID[1]
        self.iterm[2]=self.iterm[2]+self.errors_PID[2]
        
        self.func_pid[0]= (self.Kp[0] * self.errors_PID[0]) + ( self.Kd[0] *(self.errors_PID[0]-self.prev_error[0])) + (self.Ki[0]*self.iterm[0])
        self.func_pid[1]= (self.Kp[1] * self.errors_PID[1]) + ( self.Kd[1] *(self.errors_PID[1]-self.prev_error[1])) + (self.Ki[1]*self.iterm[2])
        self.func_pid[2]= (self.Kp[2] * self.errors_PID[2]) + ( self.Kd[2] *(self.errors_PID[2]-self.prev_error[2])) + (self.Ki[2]*self.iterm[1])
        #print(self.func_pid)

        #roll pwm setselfself

        if (self.func_pid[0]>0):
            self.pwm_cmd.prop1=self.setpoint_base_speed + self.func_pid[0]
            self.pwm_cmd.prop2=self.setpoint_base_speed 
            self.pwm_cmd.prop3=self.setpoint_base_speed
            self.pwm_cmd.prop4=self.setpoint_base_speed + self.func_pid[0]
        else:
            self.pwm_cmd.prop1=self.setpoint_base_speed 
            self.pwm_cmd.prop2=self.setpoint_base_speed + self.func_pid[0]
            self.pwm_cmd.prop3=self.setpoint_base_speed + self.func_pid[0]
            self.pwm_cmd.prop4=self.setpoint_base_speed

        #pitch
        if (self.func_pid[1]>0):
            self.pwm_cmd.prop1=self.setpoint_base_speed 
            self.pwm_cmd.prop2=self.setpoint_base_speed + self.func_pid[1]
            self.pwm_cmd.prop3=self.setpoint_base_speed + self.func_pid[1]
            self.pwm_cmd.prop4=self.setpoint_base_speed 
        else:
            self.pwm_cmd.prop1=self.setpoint_base_speed + self.func_pid[1]
            self.pwm_cmd.prop2=self.setpoint_base_speed 
            self.pwm_cmd.prop3=self.setpoint_base_speed 
            self.pwm_cmd.prop4=self.setpoint_base_speed + self.func_pid[1]
        ...
        #self.rollerrorpun = self.errors_PID[0]
        ...
        self.prev_error[0]=self.errors_PID[0]
        self.prev_error[1]=self.errors_PID[1]
        self.prev_error[2]=self.errors_PID[2]
        

        self.rollerrorpun =self.errors_PID[0]
        self.pitcherrorpub = self.errors_PID[1]
        self.zeroerrorpub = 0.0
        self.roll_error.publish(self.rollerrorpun)
        self.pitch_error.publish(self.pitcherrorpub)
        self.zero_error.publish(self.zeroerrorpub)
